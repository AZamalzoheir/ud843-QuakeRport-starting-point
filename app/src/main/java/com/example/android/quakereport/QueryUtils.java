...
public final class QueryUtils {
    ...
    public static  ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featured=root.getJSONArray("Feature");
            for(int i=0;i<featured.length();i++) {
            JSONObject Earthquake=featured.getJSONObject(i);
                JSONObject proparites=Earthquake.getJSONObject("properties");
                String magniuted=Double.toString(proparites.getDouble("mag"));
                String loaction=proparites.getString("place");
                String date=Long.toString(proparites.getLong("time"));
                Log.d(loaction,"first elernt in arraylist");
                earthquakes.add(new Earthquake(magniuted,loaction,date));
            }
